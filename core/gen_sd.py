...
def fit_image(image_path, width, height, out_dir=None) -> str:
    ...
    (resize+fill / letterbox — preserves aspect, pads the remainder black).

    Native models (Flux/Z-Image/Qwen) go through Wan2GP's queue, which resizes an
    image_guide to the target resolution by *stretching* it. Feeding a portrait
    guide into a square (or differently-shaped) target therefore squashes the
    subject — across a chain of img2img hops that compounds into the 'massive head'
    distortion. Pre-fitting the guide to the exact target size makes Wan2GP's
    resize a no-op, so the proportions survive. Returns the original path unchanged
    when it already matches (no needless re-encode)."""
    ...
    iw, ih = img.size
    scale = min(width / iw, height / ih)            # contain (no crop, no stretch)
    nw, nh = max(1, round(iw * scale)), max(1, round(ih * scale))
    canvas = Image.new("RGB", (width, height), (0, 0, 0))
    canvas.paste(img.resize((nw, nh), Image.LANCZOS),
                 ((width - nw) // 2, (height - nh) // 2))
    out = Path(out_dir) if out_dir else (paths.cache_dir() / "fit")
    out.mkdir(parents=True, exist_ok=True)
    f = out / f"fit_{int(time.time() * 1000)}_{width}x{height}.png"
    canvas.save(f)
    return str(f)
...
def generate_img2img(checkpoint_path, image_path, prompt, negative, width, height,
                     ...
                     batch_size=1, clip_skip=1, out_dir=None, loras=None,
                     resize_mode=2) -> list[str]:
    """Reimagine an init image with an SD-family checkpoint (img2img).

    resize_mode controls how a source whose aspect != target is fitted (see
    sd_pipeline._resize_image): 0=stretch (distorts — never default this),
    1=crop-to-fit, 2=resize+fill/letterbox (default — preserves the subject's
    proportions so a portrait fed into a square model isn't squashed into a
    'big head'; the padding is regenerated by the denoise pass)."""
    import random as _random
    pipe = get_pipeline()
    if seed is None or int(seed) < 0:
        seed = _random.randint(0, 2**31 - 1)
