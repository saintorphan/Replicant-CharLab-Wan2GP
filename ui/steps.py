...
from ..core import discovery, paths, poses
...
def build_poses(visible: bool, init=None):
    with gr.Group(visible=visible, elem_classes="replicant-step") as g:
        ...
        # Holds the base image as it was BEFORE the last "Set as Base" (➕ on a pose),
        # so the base thumbnail's ↩ can revert it.
        c["pose_base_prev"] = gr.State(None)
        with gr.Row():
            with gr.Column(scale=0, min_width=160):
                c["pose_base_thumb"] = gr.Image(
                    value=_init_img(init, "base.selected_base"),
                    label="Current base", height=150, interactive=False,
                    show_download_button=False, show_fullscreen_button=True)
                c["pose_base_undo"] = gr.Button("↩ Undo base change", size="sm",
                                                min_width=150)
        ...
        # Body-swap + ADetailer are SDXL-only passes (diffusers IP-Adapter inpaint),
        # but the POSE can be generated by any model (incl. native Flux/Z-Image). This
        # picks the SDXL checkpoint those enhancement passes use, so a Z-Image pose can
        # still get a body double / detail pass. (When the main model IS SDXL it's used
        # automatically; this is only needed for native pose models.)
        _sdxl = [(m["name"], m["path"]) for m in discovery.discover_sdxl_models()]
        c["pose_sdxl_model"] = gr.Dropdown(
            _sdxl, label="Body-swap / detail model (SDXL — for native pose models)",
            value=(_sdxl[0][1] if _sdxl else None))
                ...
                for idx in range(r, min(r + 4, n)):
                    with gr.Column(scale=1, min_width=180):
                        ...
                        with gr.Row():
                            ...
                            sb = gr.Button("➕", scale=0, min_width=36,
                                           elem_classes="replicant-pose-setbase")
                            un = gr.Button("↩", scale=0, min_width=36,
                                           elem_classes="replicant-pose-undo")
                            ab = gr.Button("⛔", variant="stop", scale=0, min_width=36,
